...
async function handleAccountsChanged(accounts) {
  if (!window.isWalletConnect) {
    ...
    localStorage.setItem(
      'key_user',
      JSON.stringify({ user: true, did: address, bscAddress: address })
    );
...
export async function onSaleBatch(
  nftType,
  nftAddresss,
  nftids,
  values,
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc721Platform.onSaleBatch(
      nftAddresss,
      nftids,
      prices,
      fee,
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc1155Platform.onSaleBatch(
      nftAddresss,
      nftids,
      prices,
      fee,
...
export async function onSale(
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc721Platform.onSale(
      nftAddress,
      nftid,
      value,
      fee,
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc1155Platform.onSale(
      nftAddress,
      nftid,
      value,
      fee,
...
export async function couponOnSale(
  nftType,
  nftAddress,
  nftid,
  value,
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc721CouponPlatform.onSale(
      nftAddress,
      nftid,
      value,
      couponCode,
    ...
    let result = await erc1155CouponPlatform.onSale(
      nftAddress,
      nftid,
      value,
      couponCode,
...
export async function auction_onsale(
  nftType,
  nftAddress,
  nftid,
  startTimeStamp,
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc721Auction.onSale(
      nftAddress,
      nftid,
      startTimeStamp,
      endTimeStamp,
    ...
    // value = web3Obj.utils.toWei(value.toString());
    fee = web3Obj.utils.toWei(fee.toString());
    let result = await erc1155Auction.onSale(
      nftAddress,
      nftid,
      startTimeStamp,
      endTimeStamp,
...
export async function revokeApprovesWithArray(
  nftType,
  tokenArr,
  value,
  onAssetIds
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    value = web3Obj.utils.toWei(value.toString());
    let result = await erc1155Platform.revokeApprove(
      tokenArr,
      onAssetIds,
      value
    );
...
export async function transferFrom(nftType, toAddress, nftId, amountValue) {
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    let transferFrom = await uptick1155.safeTransferFrom(
      toAddress,
      nftId,
      amountValue
    );
...
export async function lazyNftMint(
  toAddress,
  nftId,
  metaDataUrl,
  payAddress,
  ...
  let mintFrom = await lazyNFT.lazyMint(
    toAddress,
    nftId,
    metaDataUrl,
    payAddress,
...
export async function lazyNft1948Mint(
  toAddress,
  tokenId,
  baseurl,
  payAddress,
  ...
  let mintFrom = await lazyNFT1948.lazyMint(
    toAddress,
    tokenId,
    baseurl,
    payAddress,
...
export function setContractAddress(nftType, addressObject) {
  ...
  ierc20.setContractAddress(
    addressObject.token20Address,
    addressObject.platformAddress
  );
...
export async function placeOrder(
  ...
  if (nftType == 'ERC721' || nftType == 'ERC1948') {
    ...
    if (marketType == 3 || marketType == 5) {
      transferFrom = await erc721CouponPlatform.placeOrder(
        nftAddress,
        nftId,
        toAddress,
        price,
      ...
      transferFrom = await erc721Platform.placeOrder(
        nftAddress,
        nftId,
        toAddress,
        price
    ...
    let transferFrom;
    if (marketType == 3 || marketType == 5) {
      transferFrom = await erc1155CouponPlatform.placeOrder(
        nftAddress,
        nftId,
        toAddress,
        price,
      ...
      transferFrom = await erc1155Platform.placeOrder(
        nftAddress,
        nftId,
        toAddress,
        price
...
export async function uptickCrossToEVM(
  srcChainName,
  destinationChainId,
  toAddress,
  metadate,
