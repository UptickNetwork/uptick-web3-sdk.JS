...
export async function transfer(owner, tokenId, value, assetId) {
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .transfer(owner, tokenId, value, amount, address, utils.toUtf8Bytes(data))
      .encodeABI();
...
export async function onSale(
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .onSale(nftAddress, nftid, value, amount, payAddress)
      .encodeABI();
    let result = await wallectConnectSendTransaction(
      fromAddress,
      contractAddress,
...
export async function onSaleBatch(
  ...
  if (!hasWalletConnect) {
    ...
    let rep = await contract.onSaleBatch(
      nftAddresss,
      nftids,
      values,
      amounts,
    ...
    let data = contract.methods
      .onSaleBatch(nftAddresss, nftids, values, amounts, payAddresss)
      .encodeABI();
    let result = await wallectConnectSendTransaction(
      fromAddress,
      contractAddress,
...
export async function offSale(nftAddress, nftid) {
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .offSale(nftAddress, nftid, fromAddress)
      .encodeABI();
...
export async function offSaleBatch(nftAddress, nftids) {
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .offSaleBatch(nftAddress, nftids, fromaddressarr)
      .encodeABI();
...
export async function revokeApprove(tokenArr, onAssetIds, value) {
  ...
  if (!hasWalletConnect) {
    let gasSetting = await base.getGasPriceAndGasLimit();
    let rep = await contract.revokeApprove(
      tokenArr[0],
      value,
      amount,
      utils.toUtf8Bytes(data),
    ...
    let data = contract.methods
      .revokeApprove(tokenArr[0], value, amount, utils.toUtf8Bytes(data))
      .encodeABI();
