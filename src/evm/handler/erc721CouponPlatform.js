...
import { abi } from '../abi/ERC721CouponPlatform.json';
...
export async function onSale(
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .onSale(nftAddress, nftid, value, payAddress, couponCode, reducedPrice)
      .encodeABI();
...
export async function placeOrder(
  ...
  if (!hasWalletConnect) {
    ...
    let data = contract.methods
      .placeOrder(nftAddress, nftId, fromAddress, couponCode, couponLink)
      .encodeABI();
    let result = await wallectConnectSendTransaction(
      fromAddress,
      contractAddress,
